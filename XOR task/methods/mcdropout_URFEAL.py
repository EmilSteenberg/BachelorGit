...
from functions import set_seed, NN, model_definition, train_one_epoch, validate_model, mc_dropout_predict, uncertainty_score_from_var, clusterer, run_OFE_regression

def run_MC_URF(X_train, t_train, X_val, t_val, data_loss,
                L, budget, SEED, NN_config, epochs=[20, 10], radius=0.2, beta=10, use_OFE=False):
    ...
    for epoch in range(epochs[0]):
        train_loss = train_one_epoch(model, optimizer, X_train[labeled_idx], t_train[labeled_idx], data_loss)
        if epoch == 0:
    ...
    while len(labeled_idx) < target_count:
        ...
        _, var_u, features = mc_dropout_predict(model, X_u, T=30, return_features=True)
        ...
        for epoch in range(epochs[1]):
            train_loss = train_one_epoch(model, optimizer, 
                                            X_train[labeled_idx], t_train[labeled_idx], data_loss)
