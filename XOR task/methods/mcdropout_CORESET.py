...
def run_MCpCAL(X_train, t_train, X_val, t_val, data_loss, L, budget, b, SEED, NN_config, epochs=[20, 10]):
    ...
    while len(labeled_idx) < target_count:
        ...
        mean_u, var_u = mc_dropout_predict(model, X_u, T=30)
        ...
        for epoch in range(epochs[1]):
            train_loss = train_loss = train_one_epoch(model, optimizer, X_train[labeled_idx], t_train[labeled_idx], data_loss)
